...
class HostGuestTopology:
    ...
    def parameterize_nonbonded(
        ...
        guest_ixn_env_params, guest_pot = self.guest_topology.parameterize_nonbonded(
        ...
        combined_exclusion_idxs = np.concatenate([exclusion_idxs, guest_exclusions])
        combined_scale_factors = np.concatenate([scale_factors, guest_scale_factors])
        nb_params = jnp.concatenate([self.host_nonbonded.params, guest_ixn_env_params])

        nb_pot = potentials.Nonbonded(
...
class MultiTopology(BaseTopology):
    ...
    def _parameterize_nonbonded(
        ...
        for mol in self.mols:
            if intramol_params:
                q_params = self.ff.q_handle_intra.partial_parameterize(ff_q_params_intra, mol)
                lj_params = self.ff.lj_handle_intra.partial_parameterize(ff_lj_params_intra, mol)
            else:
                q_params = self.ff.q_handle.partial_parameterize(ff_q_params, mol)
                lj_params = self.ff.lj_handle.partial_parameterize(ff_lj_params, mol)

            q_mol_parameters.append(q_params)
            lj_mol_parameters.append(lj_params)
            exclusion_idxs, scale_factors = nonbonded.generate_exclusion_idxs(
                mol, scale12=_SCALE_12, scale13=_SCALE_13, scale14_q=_SCALE_14_Q, scale14_lj=_SCALE_14_LJ
            )
            mol_exclusions.append(exclusion_idxs + offset)
            mol_scales.append(scale_factors)
        ...
        combined_exclusion_idxs = np.concatenate([*mol_exclusions, mutual_exclusions]).astype(np.int32)
    ...
    def _parameterize_bonded_term(self, ff_params, bonded_handle, potential):
        ...
        mol_idxs = []
        for mol in self.mols:
            params, idxs = bonded_handle.partial_parameterize(ff_params, mol)
            mol_parameters.append(params)
            mol_idxs.append(idxs + offset)
            offset += mol.GetNumAtoms()
