...
from tmd.fe import interpolate, model_utils, topology, utils
from tmd.fe.chiral_utils import ChiralRestrIdxSet
from tmd.fe.dummy import (
    canonicalize_bond,
...
def find_dummy_groups_and_anchors(
    ...
    Refer to :py:func:`tmd.fe.dummy.generate_dummy_group_assignments` and notes below for more
...
class SingleTopology(AtomMapMixin):
    ...
    def combine_masses(self, use_hmr: bool = False) -> list[float]:
        ...
        use_hmr: bool
            Applies HMR to the masses. Refer to tmd.fe.model_utils.apply_hmr for details.
    ...
    def _parameterize_host_guest_nonbonded_ixn(
        ...
        hg_nb_params = jnp.concatenate([host_params, guest_ixn_env_params])

        guest_exclusions, guest_scale_factors = exclude_all_ligand_ligand_ixns(num_host_atoms, num_guest_atoms)

        n_atoms = num_host_atoms + num_guest_atoms
