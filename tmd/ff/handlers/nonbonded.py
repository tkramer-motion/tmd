...
from tmd.ff.handlers.utils import match_smirks as rd_match_smirks
...
def oe_assign_charges(mol, charge_model: str = AM1BCCELF10) -> NDArray:
    ...
    # https://github.com/badisa/tmd#forcefield-gotchas
...
def amber_am1bcc_assign_charges(mol: Chem.Mol) -> NDArray:
    # Generate conformations
    copy_mol = Chem.Mol(mol)
    # RMS Threshold set to the same value as the prune_conformers_elf of 0.5 angstroms
    generate_conformations(copy_mol, rms_threshold=0.5)

    pruned_mol = prune_conformers_elf(copy_mol)
    charge_sets = []
    for conf in pruned_mol.GetConformers():
        # Setup copy with a single conf
        copy = Chem.Mol(copy_mol)
        copy.RemoveAllConformers()
        copy.AddConformer(Chem.Conformer(conf))

        try:
            # Sometimes AM1BCC will fail to charge a conformer
            conf_charges = amber_am1bcc_charge_mol(copy)
            charge_sets.append(conf_charges)
        except Exception as e:
            warnings.warn(f"Conformer failed to charge: {e!s}")
            continue
    if len(charge_sets) == 0:
        raise RuntimeError(f"Unable to assign charges for '{get_mol_name(mol)}'")
    mean_partial_charges = np.mean(np.array(charge_sets), axis=0)
    assert mean_partial_charges.size == copy_mol.GetNumAtoms()

    expected_charge = Chem.GetFormalCharge(copy_mol)
    current_charge = np.sum(mean_partial_charges)
    charge_offset = (expected_charge - current_charge) / copy_mol.GetNumAtoms()
    # Add an assertion on how small the offset can be??
    mean_partial_charges += charge_offset

    net_charge = np.sum(mean_partial_charges)
    assert np.isclose(net_charge, expected_charge, atol=1e-5), f"Charge is not an integer: {net_charge}"

    # Apply scaling factor for TMD's optimized charge representation
    # https://github.com/badisa/tmd#forcefield-gotchas
    # "The charges have been multiplied by sqrt(ONE_4PI_EPS0) as an optimization."
    inlined_constant = np.sqrt(constants.ONE_4PI_EPS0)

    # returned charges are in TM units, in original atom ordering
    return inlined_constant * mean_partial_charges


def amber_am1bcc_charge_mol(mol: Chem.Mol) -> NDArray:
    assert len(mol.GetConformers()) == 1, "Mol can only have one conformer"
    antechamber_path = which("antechamber")
    if antechamber_path is None:
        raise RuntimeError("Antechamber couldn't be found")
    # Compute AM1BCC charges using Antechamber
    with TemporaryDirectory() as tmpdir:
        ...
        assert (Path(tmpdir) / charges_output).is_file()
        # Read the charges
        with open(Path(tmpdir) / charges_output) as ifs:
            text_charges = ifs.read().split()
        charges = np.array([float(charge) for charge in text_charges])
    return charges
...
class AmberAM1BCCHandler(SerializableMixIn):
    """The AmberAM1BCCHandler generates charges for molecules using AmberTools's Antechamber AM1BCC protocol.
    ...
    def static_parameterize(mol):
        ...
        """
        return compute_or_load_amber_am1bcc_charges(mol)


class AmberAM1BCCIntraHandler(AmberAM1BCCHandler):
    pass
